    ...
    setupEventListeners() {
        ...
        if (this.totalSlides > 1) {
            this.segment.addEventListener('click', (e) => {
                e.stopPropagation();
                this.togglePause();
                actualizarContadorGuardar(); // Actualizar contador
            });
        }
    ...
    setupEventListeners() {
        ...
        if (deleteBtn) {
            deleteBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                e.preventDefault();
                removeAccessory(parseInt(this.container.dataset.id));
                actualizarContadorGuardar(); // Actualizar contador
            });
        }
    }
...
function initializeDefaultAccessories() {
    ...
    if (window.accesoriosImagenes && window.accesoriosImagenes.length > 0) {
        ...
        window.accesoriosImagenes.forEach((img, index) => {
            if (index < 0) { // Máximo 3 accesorios
                ...
                if (accessory) {
                    ...
                    if (carouselIndex >= 0) {
                        ...
                        accessoryCarousels[carouselIndex].setupEventListeners();
                    }
                }
            }
        });
    }
    actualizarContadorGuardar(); // Actualizar contador inicial
}
...
// ============================================
// FUNCIONES PARA GUARDAR OUTFIT (MODIFICADAS)
// ============================================

function obtenerPrendasActivas() {
    const prendas = [];

    // Obtener solo prendas de segmentos PAUSADOS
    document.querySelectorAll(".segment-image.active").forEach(img => {
        const segmento = img.closest('.model-segment');
        
        // Verificar si el segmento está pausado (tiene la clase 'paused')
        if (segmento && segmento.classList.contains('paused')) {
            prendas.push({
                imagen: img.src,
...
function obtenerAccesoriosActivos() {
    ...
    document.querySelectorAll(".accessory-container").forEach(container => {
        // Verificar si el accesorio está pausado
...
function actualizarContadorGuardar() {
    ...
    const total = prendas.length + accesorios.length;
    
    if (total > 0) {
        guardarBtn.innerHTML = `<i class="fas fa-save"></i> Guardar Outfit (${total})`;
        guardarBtn.style.backgroundColor = '#4CAF50'; // Verde cuando hay elementos
        guardarBtn.style.color = 'white';
    } else {
        guardarBtn.innerHTML = `<i class="fas fa-save"></i> Guardar Outfit`;
        guardarBtn.style.backgroundColor = ''; // Color original
        guardarBtn.style.color = '';
    }
}
...
async function guardarOutfit() {
    ...
    console.log("Prendas y accesorios PAUSADOS para guardar:", todas);

    if (todas.length === 0) {
        alert("⚠️ No hay elementos pausados para guardar.\n\n👉 Haz clic en una prenda o accesorio para pausarlo (detener el carrusel) antes de guardar.");
        return;
    }

    // Mostrar confirmación con detalles
    let confirmMessage = `¿Guardar este outfit con ${todas.length} elemento(s)?\n\n`;
    
    if (prendas.length > 0) {
        confirmMessage += "📌 Prendas:\n";
        confirmMessage += prendas.map(p => `  • ${p.tipo}`).join('\n');
    }
    
    if (accesorios.length > 0) {
        confirmMessage += (prendas.length > 0 ? '\n\n' : '') + "💎 Accesorios:\n";
        confirmMessage += accesorios.map(a => `  • ${a.tipo}`).join('\n');
    }
    ...
    formData.append("estilo", "Personalizado");
    formData.append("clima_recomendado", "Templado");
    ...
    try {
        ...
        if (result.success) {
            ...
            // Despausar los elementos después de guardar
            document.querySelectorAll('.model-segment.paused').forEach(segment => {
                segment.classList.remove('paused');
                // Reanudar el carrusel si corresponde
                const segmentId = segment.dataset.segmentId;
                if (window.mainCarousel && window.mainCarousel.segments[segmentId]) {
                    window.mainCarousel.segments[segmentId].resume();
                }
            });
            
            document.querySelectorAll('.accessory-container.paused').forEach(acc => {
                acc.classList.remove('paused');
                // Reanudar el carrusel del accesorio
                const carousel = accessoryCarousels.find(c => c.container === acc);
                if (carousel) {
                    carousel.resume();
                }
            });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.mainCarousel = mainCarousel; // Hacerlo global
    ...
    if (toggleAccessoriesBtn && accessoriesPanel && addAccessoryBtn && accessoriesGrid) {
        ...
        addAccessoryBtn.addEventListener("click", function() {
            ...
            if (!accessoriesPanel.classList.contains("active")) {
                accessoriesPanel.classList.add("active");
                toggleAccessoriesBtn.innerHTML = '<i class="fas fa-minus-circle"></i> Ocultar accesorios';
            }
        });
    }
